...
| **8. Rootfs propio** | Buildroot controlado + picoarch + TreeFrogUI + fix AVP-media + clean install | CLEAN-INSTALL PHYSICAL PASS | ✅ **DONE DEFINITIVO 2026-09-18 (bisect completo)**: rootfs propio 10,4 MiB (8a-8c), launcher fábrica eliminado (8d), boot ~8s (8f); **AVP-media COMPLETO: audio+video+salida-cores PHYSICAL PASS (9l/9m — drift ABI ADR-012)**; **8e: stack relocalizado a `treefrog/` + CLEAN-INSTALL PHYSICAL PASS** — cubegm = 10 archivos (boot 5 + 4 pre-Linux-required de fábrica [setting.xml/xgame-logo.bmp/allfiles.lst/root.dat] + flag diag); causa raíz del primer fail aislada por bisect de 3 boots; diag opt-in (ADR-013) validado físicamente |
