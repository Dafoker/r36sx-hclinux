...
3. **Memory layout stock real:** DTB stock `memory reg = <0x00 0xaf91e50>` → **Linux = ~184 MB** de los 256 MB (AVP/MMZ se lleva ~72 MB). El DTS SDK d3100_v20 da 254 MB a Linux → **usar DTB SDK en la consola real pisaría memoria del AVP (riesgo brick/brick funcional)**. Este valor DEBE modelarse en la board propia.
   > **CORRECCIÓN Fase 4A (2026-09-14):** los valores precisos son Linux **175.57 MiB** / AVP **80.43 MiB** (no "~184/~72"); y el SDK d3100_v20 real es **128 MiB total / Linux 79.20 MiB** (el "254 MB" provenía de macros del avp.dtsi v10, no del v20). Tabla exacta: docs/DTS_STOCK_MODEL.md §mapa-memoria. La conclusión operativa se mantiene: memory maps incompatibles → DTS propio obligatorio.
