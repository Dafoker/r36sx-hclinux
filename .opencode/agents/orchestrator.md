...
- **KERNEL PROVENANCE GATE (AGENTS.md §14):** ningún kernel BUILD PASS habilita la fase siguiente sin evidencia conservada de: host/target arch, cross compiler REALMENTE invocado (.cmd de kbuild, no suposición), triplet/versión/sysroot, ARCH/CROSS_COMPILE efectivos, patch set aplicado con orden (log, no presencia), inyección BSP documentada, ELF kernel+userspace+módulos, hashes. Presence ≠ use; vermagic ≠ toolchain. Gates `scripts/audit_toolchain.sh` + `scripts/audit_kernel_patches.sh` deben PASS antes de Fase 4.
